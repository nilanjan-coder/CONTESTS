...
long long MOD = 1e9 + 7, LIM = 2e5;
vector <long long> f(LIM + 1);
void compute() {
	for (long long i = 0; i <= 8; i ++) {
		f[i] = 2;
	}
	f[9] = 3;
	for (long long i = 10; i <= LIM; i ++) {
		f[i] = (f[i - 9] + f[i - 10]) % MOD;
	}
}
vector <long long> int_to_arr (long long x) {
	vector <long long> a;
	while (x) {
		a.push_back(x % 10);
		x /= 10;
...
void solve() {
	...
	scanf ("%lld%lld", &n, &m);
	//cin >> n >> m;
	vector <long long> a = int_to_arr(n);
	long long ans = 0;
	for (long long d: a) {
		long long op = 10 - d;
		op = m - op;
		ans = (ans + ((op < 0) ? 1 : f[op])) % MOD;
	}
	printf ("%lld\n", ans);
	//cout << ans << endl;
}
