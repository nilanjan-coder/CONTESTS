...
void bin (vll &a, vll &s, ll start, ll end) {
	sum.ins(s[end] - s[start-1]);
	if (end == start) return;
	ll val = (a[end] + a[start]) / 2;
	ll mid = ub(all(a),val) - a.B;
	if (mid == end + 1 || mid == start) return;
	bin(a,s,start,mid-1);
	bin(a,s,mid,end);
}
int main () {
