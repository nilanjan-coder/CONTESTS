...
//For graph problems, make sure to declare everything globally. If multiple test cases, then reinit everything inside init function.
...
int main() {
	...
	if (k & 1) {
		...
		k /= 2;
		vector <vector <vector <int>>> a(n + 1, vector <vector <int>> (m + 1, vector <int> (k + 1, 0)));
		for (int p = 1; p <= k; p ++) {
			for (int i = 1; i <= n; i ++) {
				for (int j = 1; j <= m; j ++) {
					a[i][j][p] = min ({(i > 1) ? (a[i - 1][j][p - 1] + v[i - 1][j]) : INF, (j > 1) ? (a[i][j - 1][p - 1] + h[i][j - 1]) : INF,
										(i < n) ? (a[i + 1][j][p - 1] + v[i][j]) : INF, (j < m) ? (a[i][j + 1][p - 1] + h[i][j]) : INF});
				}
			}
		}
		for (int i = 1; i <= n; i ++) {
