//g++ -Wall -Wextra -pedantic -std=c++17 -O2 -Wshadow -Wformat=2 -Wfloat-equal -Wconversion -Wlogical-op -Wshift-overflow=2 -Wduplicated-cond -Wcast-qual -Wcast-align -D_GLIBCXX_DEBUG -D_GLIBCXX_DEBUG_PEDANTIC -D_FORTIFY_SOURCE=2 -fsanitize=address -fsanitize=undefined -fno-sanitize-recover -fstack-protector A.cpp
#include <bits/stdc++.h>
using namespace std;
...
long long sum_of_dig (long long n) {
	long long s = 0;
	while (n) {
		s += (n % 10);
		n /= 10;
...
void solve() {
	...
	cin >> n;
	while (true) {
		long long gcd = __gcd(n, sum_of_dig(n));
		if (gcd == 1) {
			n ++;
		}
		else {
