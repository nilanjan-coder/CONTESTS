...
bool cmp2 (pair <int, vector <int>> p1, pair <int, vector <int>> p2) {
	return p1.first < p2.first;
}
void solve() {
	int n, m, x;
	...
	vector <pair <int, vector <int>>> tower(m, {0, {}});
	int left = n, start = 0;
	while (left) {
		if (left >= m) {
			for (int i = start; i < start + m; i ++) {
				tower[i - start].first += h[i].first;
				tower[i - start].second.push_back(h[i].second);
			}
			left -= m;
			start += m;
		}
		...
		sort (tower.begin(), tower.end(), cmp2);
	}
	vector <int> y(n);
	for (int i = 0; i < m; i ++) {
		for (int idx: tower[i].second) {
			y[idx] = i;
		}
	}
	cout << "YES" << endl;
