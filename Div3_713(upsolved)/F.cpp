...
void solve() {
	...
	dp[0].second = 0;
	for (int i = 1; i < n; i ++) {
		long long x = (b[i - 1] > dp[i - 1].first) ? ceil ((b[i - 1] - dp[i - 1].first) / (a[i - 1] * 1.0)) : 0;
		dp[i].first = dp[i - 1].first + x * a[i - 1] - b[i - 1];
		dp[i].second = dp[i - 1].second + x + 1;
	}
	long long ans = 1e18;
	for (int i = 0; i < n; i ++) {
		ans = min (ans, dp[i].second + (long long) ceil ((c - dp[i].first) / (a[i] * 1.0)));
	}
	cout << ans << endl;
	//dbg(dp);
}
